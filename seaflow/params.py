...
class ParamDefinitionItem(object):
    def __init__(self):
        """
        required与default: 和drf serializer/python函数定义中的参数列表一致
            required=True, 则不用设置default, 参数缺失时抛出错误
            required=False, 参数缺失时, 如果设置了default则取default的值. 如果没有设置default则不赋值
        """
        self.type = None
        self.required = None
        self.default = None
    ...
    def from_json(cls, definition_item: dict):
        ...
        r.type = get_param_class(definition_item.get('type'))
        assert issubclass(r.type, ParamType)

        r.required = definition_item.get('required', True)

        if 'default' in definition_item:
            r.default = definition_item['default']
            assert r.type.is_valid(r.default)
            r.set_default = True

        return r

    def to_json(self):
        return {'type': self.type.__name__, 'required': self.required, 'default': self.default, 'set_default': self.set_default}
...
if __name__ == '__main__':
    ...
    _adapter = ParamAdapter.from_json({'a': '$.aa', 'b': '$.bb', 'c': '$.cc'})
    r = _adapter.parse({'aa': [1, 2], 'bb': {'x': 1}, 'cc': 12})
