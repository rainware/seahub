...
class Dag(BaseModel):
    ...
    # 跳过
    # skipable = models.BooleanField('可跳过', default=False)
    # skip_config = models.JSONField(default=dict)
...
class Task(BaseModel):
    ...
    def to_dict(self, with_logs=False, brief=False):
        if brief:
            d = {
                'id': self.id,
                'name': self.name,
        ...
        if brief:
            d = {
                ...
                'node': {
                    'id': self.node.id,
                    'name': self.node.name,
                    'dag_id': self.node.dag_id,
                    'root_dag_id': self.node.root_dag_id
                },
                'root_id': self.root_id,
